  ...
  return (
    <div className="min-h-screen bg-black text-white p-4">
      ...
      <div className="mb-6">
        ...
        <p className="text-gray-400 text-sm mb-4">Showing all items fetched by useTVFeed hook</p>
          ...
          >
            ...
            <div className="aspect-square relative bg-gray-800">
              ...
              <div className="absolute top-1 right-1 flex gap-1">
                {item.videoUrl && (
                  <span className="bg-green-500/80 px-1.5 py-0.5 rounded text-xs">VIDEO</span>
                )}
                {item.isDroposal && (
                  <span className="bg-orange-500/80 px-1.5 py-0.5 rounded text-xs">DROP</span>
                )}
                {item.poolCurrencyTokenAddress?.toLowerCase() === GNARS_COIN_ADDRESS && (
                  <span className="bg-purple-500/80 px-1.5 py-0.5 rounded text-xs">GNARS</span>
                )}
              </div>
            </div>
            ...
            <div className="p-2">
              ...
              {item.symbol && <p className="text-xs text-yellow-400 truncate">${item.symbol}</p>}
              {item.marketCap && (
                <p className="text-xs text-gray-400">MC: ${(item.marketCap / 1e18).toFixed(2)}</p>
              )}
              {item.coinAddress && (
                <a
      ...
      {filteredItems.length === 0 && (
        <div className="text-center text-gray-500 py-12">No items match the current filter</div>
      )}
    </div>
  );
}

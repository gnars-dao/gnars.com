...
import {
  useDeploySponsorshipVault,
  type DeployPhase,
  type DeployResult,
} from "@/hooks/use-deploy-sponsorship-vault";
import { isVaultAdmin, RIDER_LIST, type RiderId } from "@/lib/gnars-vaults";
...
export function StakeAdminPanel() {
      ...
      <Card className="border-border/60">
        <CardHeader>
          <CardTitle className="text-sm text-muted-foreground">
            Sponsorship vaults (admin)
          </CardTitle>
          <CardDescription className="font-mono text-xs">
            connected, but no admin permission.
            <br />
            active: {activeAddr ?? "—"}
            <br />
  ...
  async function onDeploy(id: RiderId, wallet: string, existingSplit?: string) {
    ...
    const out = await deploy(
      wallet as `0x${string}`,
      id,
      existingSplit as `0x${string}` | undefined,
    );
    if (out) {
      setResults((r) => ({ ...r, [id]: out }));
      toast.success(`${id} vault deployed`, {
        description: "Approve the batch in the SOPA Safe to activate.",
      });
    } else {
    ...
    <Card className="border-yellow-500/30 bg-yellow-500/[0.03]">
      <CardHeader>
        ...
        <CardDescription>
          ...
          Gnars&nbsp;25% / athlete&nbsp;25% (the depositor keeps 50%). Owner = SOPA Safe. You sign
          the deploy and the batch — 1 of the Safe’s 2 confirmations.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-2">
        {RIDER_LIST.map((r) => {
          const done =
            results[r.id] ??
            (r.vault
              ? ({ vault: r.vault, adapter: r.adapter, split: r.split } as DeployResult)
              : undefined);
              ...
              <div className="flex items-center justify-between gap-3">
                ...
                {!done && r.wallet && (
                  <Button
                    ...
                    onClick={() => onDeploy(r.id, r.wallet!, r.split)}
                  >
        ...
        {Object.keys(results).length > 0 && (
          <p className="pt-1 text-[11px] text-muted-foreground">
            Paste the addresses above into <code>src/lib/gnars-vaults.ts</code> (the rider’s
            vault/adapter/split) to light up the sponsorship on /stake.
          </p>
        )}
      </CardContent>
    </Card>

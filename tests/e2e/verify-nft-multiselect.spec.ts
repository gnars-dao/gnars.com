...
test("multi-NFT select in proposal builder", async ({ page }) => {
  ...
  const titleInput = page
    .locator(
      'input[name="title"], textarea[name="title"], input[placeholder*="title" i], input[placeholder*="Title" i]',
    )
    .first();
  ...
  const nextBtn = page
    .locator(
      'button:has-text("Next"), button:has-text("Transactions"), [role="tab"]:has-text("Transactions")',
    )
    .first();
  ...
  const sendNftsCard = page
    .locator('button:has-text("Send NFTs"), [role="button"]:has-text("Send NFTs")')
    .first();
  ...
  const submitBtn = page
    .locator('button:has-text("Add 2 transactions"), button:has-text("Add")')
    .first();
  ...
  if (await addMoreBtn.isVisible({ timeout: 2000 }).catch(() => false)) {
    ...
    if ((await nftBtns3.count()) >= 3) {
      ...
      await page.waitForTimeout(300);
      const btn3 = page
        .locator('button:has-text("Add 3 transactions"), button:has-text("Add")')
        .first();
